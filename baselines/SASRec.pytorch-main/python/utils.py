...
def evaluate(model, dataset, args, K=10):
    ...
    for u in users:
        ...
        relevant_items = list(test[u])

        # candidate items = relevant + negative
        item_idx = relevant_items.copy()
        for _ in range(100):
            t = np.random.randint(1, itemnum + 1)
            while t in rated or t in relevant_items:
                t = np.random.randint(1, itemnum + 1)
            item_idx.append(t)
        ...
        # ranking
        ranked_idx = np.argsort(predictions)
        top_k_idx = ranked_idx[:K]
        top_k_items = [item_idx[i] for i in top_k_idx]

        # -------- Recall --------
        hit = len(set(top_k_items) & set(relevant_items))
    ...
    return NDCG / valid_user, Recall / valid_user
